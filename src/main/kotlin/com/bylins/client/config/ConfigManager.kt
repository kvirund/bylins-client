...
class ConfigManager {
    ...
    /**
     * Сохраняет триггеры и алиасы в файл
     */
    fun saveConfig(triggers: List<Trigger>, aliases: List<Alias>) {
        try {
    ...
    /**
     * Загружает триггеры и алиасы из файла
     */
    fun loadConfig(): Pair<List<Trigger>, List<Alias>> {
        try {
            ...
            println("Config loaded from: $configFile (${triggers.size} triggers, ${aliases.size} aliases)")
            return Pair(triggers, aliases)
        } catch (e: Exception) {
     ...
     */
    fun exportConfig(file: File, triggers: List<Trigger>, aliases: List<Alias>) {
        try {
            val config = ClientConfig(
                triggers = triggers.map { TriggerDto.fromTrigger(it) },
                aliases = aliases.map { AliasDto.fromAlias(it) }
     ...
     */
    fun importConfig(file: File): Pair<List<Trigger>, List<Alias>> {
        try {
            ...
            println("Config imported from: ${file.absolutePath} (${triggers.size} triggers, ${aliases.size} aliases)")
            return Pair(triggers, aliases)
        } catch (e: Exception) {
