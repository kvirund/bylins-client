...
class MapManager(
    ...
    fun setCurrentRoom(roomId: String) {
        if (_rooms.value.containsKey(roomId)) {
            ...
            // Только теперь маршрут соответствует новому месту: пройденный шаг
            // вычеркнут, сбившийся путь пересчитан. Тот, кто идёт по маршруту,
            // должен узнать об этом последним — иначе возьмёт ещё не убранный
            // шаг и повторит направление, которым только что пришёл.
            if (previousRoomId != roomId) {
                onPathAdvanced?.invoke()
            }
        }
    }

    /**
     * Вызывается после того, как смена комнаты полностью учтена в маршруте.
     * Сюда подключается автоматический проход по пути.
     */
    var onPathAdvanced: (() -> Unit)? = null
