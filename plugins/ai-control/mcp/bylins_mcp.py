...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "другой агент, вызов вернёт ошибку. Вызовы не параллелятся: пока "
            "предыдущий залп ждёт ответа, следующий получит отказ — персонаж "
            "один, и вывод двух команд перемешался бы."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": (
            ...
            "Массовые правки: create/update/delete у триггеров, алиасов, хоткеев, "
            "вкладок, профилей подключения и контекстных правил принимают пакет — "
            "params.items (массив объектов) "
