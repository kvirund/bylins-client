...
/**
 * Событие вывода, который клиент напечатал сам: эхо команд, ответы #-команд,
 * сообщения плагинов, ходока и скриптов.
 *
 * Отдельно от [LineReceivedEvent] по двум причинам: строки сервера можно
 * отменить (gag) и по ним работают триггеры, а собственный вывод уже показан
 * игроку — отменять нечего, и гонять по нему триггеры значило бы зациклиться
 * на собственном эхо.
 */
data class LocalOutputEvent(
