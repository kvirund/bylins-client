...
interface PluginAPI {
    ...
    /**
     * Ведёт игрока к комнате: клиент шлёт по одному направлению и ждёт, пока
     * игрок действительно окажется в следующей комнате.
     *
     * Отправлять маршрут самому (`send("север;север;...")`) нельзя: на первой
     * же заминке — дверь, бой, «слишком устали» — остаток команд уходит из
     * чужой комнаты, и сервер сыплет «туда не пройти». Здесь же сбившийся
     * маршрут пересчитывается, а застрявший — останавливается.
     *
     * @return false, если пути нет или идти некуда
     */
    fun startWalk(targetRoomId: String): Boolean

    /** Останавливает проход по маршруту. */
    fun stopWalk()

    /** Идёт ли сейчас проход по маршруту. */
    ...
    /**
     * Правит поля самой комнаты: name, zone, terrain, visited, notes, color.
     *
     * Меняются только переданные ключи. Пустая строка в zone/terrain/color
     * очищает поле. Нужно для мест, куда заходить нельзя (ДТ и прочие
     * ловушки), и для комнат-заготовок, известных лишь по выходу соседа:
     * пометить посещённой, дать имя и зону, не входя внутрь.
     *
    ...
    // ============================================
    // Управление клиентом (требует разрешений)
    // ============================================

    /**
     * Управление клиентом «как если бы пользователь нажимал кнопки»:
     * профили подключения, соединение, триггеры/алиасы/хоткеи, вкладки.
     *
     * Требует разрешений, выданных пользователем в настройках плагинов
     * ([PluginPermission.CLIENT_CONTROL], [PluginPermission.CONNECTION_CONTROL]);
     * иначе вызовы бросают [PluginPermissionDeniedException].
     */
    val client: ClientControl

    /**
     * Выдано ли плагину указанное разрешение.
     * Позволяет плагину заранее скрыть недоступную функциональность.
     */
    ...
    /**
     * Регистрирует собственный блок плагина в правой (статус) панели.
     *
     * В отличие от [addStatusText] и других готовых элементов, содержимое
     * описывает сам плагин — значит доступны кнопки и прочие интерактивные
     * узлы (например, «Отключить» напротив ИИ-сессии).
     *
     * Повторный вызов с тем же [id] обновляет содержимое, [removeStatus] убирает.
     */
    fun addStatusPanel(
        id: String,
        label: String,
